...
- **`src/RalfHuesing.Mcp.Observability/Internal/ArgumentSanitizer.cs`** — Rekursive Bereinigung und Redaktion sensibler Argument-Schlüssel. (zuletzt: step-001)
- **`src/RalfHuesing.Mcp.Observability/Internal/JsonlLogWriter.cs`** — Thread-sicherer Writer für JSONL-Dateien in `%LOCALAPPDATA%` oder konfigurierte Pfade. (zuletzt: step-001)
- **`tests/RalfHuesing.Mcp.Observability.Tests/Internal/ArgumentSanitizerTests.cs`** — Unit-Tests für Redaktion sensibler Schlüssel und verschachtelter Strukturen. (zuletzt: step-001)
- **`tests/RalfHuesing.Mcp.Observability.Tests/Internal/JsonlLogWriterTests.cs`** — Unit-Tests für Dateipfade, JSONL-Serialisierung und Thread-Sicherheit. (zuletzt: step-001)
